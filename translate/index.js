...
/*</.append.copy>*/
    ...
    if (new URLSearchParams(location.search).has('iframe')) {
        ...
        const translatePromise = new Promise(res => {
            ...
            i.src = 'https://hoangshiga.github.io/translate/?iframe' + (new URLSearchParams(location.search).has('debug') ? '&debug' : '')
            var promise
            window.addEventListener('message', m => {
                // console.log('parent', m.data)
                if (m.data.translateInit) res((s, res) => {
        ...
        window.addEventListener('message', async m => {
            ...
            if (m.data.returnTypeValue || m.data.onTypeValueChanged) {
                ...
                const result = await (await translatePromise)(m.data.returnTypeValue || m.data.onTypeValueChanged)
        ...
        const iframe = append(document.body, 'iframe', {
            style: 'width: 100%; height: 40%', src: 'https://hoangshiga.github.io/type/' + (new URLSearchParams(location.search).has('debug') ? '?debug' : '')
        })
